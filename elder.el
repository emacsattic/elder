;;;-*- auto-recompile: t -*-
;; elder.el----Emacs-lisp preprocessor for any documents/code. 
;; Time-stamp: <2005-11-21 18:25:20 deego>
...
;; Version: 3.4.2alpha
;; See also: 
;;   I have found 2 more libraries whose description does something
;;   similar: 
;;   elp.el: (emacs lisp list)
;;   elpp.el (see emacs lisp list)
;;  emacs-lisp preprocessing.

;;; ELDER==> the ELisp Document /preprocEssoR/writER/formattER/unitER. 
 (defvar elder-version "3.4.2alpha")
...
;;; 12000..
;;;
;;; Also consider adding (elder-editing-modes) to your .emacs
...
copied from .emacs.macros on 9/23/00.." 
 (let 
   ...
   (if (<= ll 0)
	 ...
	 (if (null firstpos)
	     (list string)
	   (cons (buffer-substring-no-properties (point-min) (- firstpos 1))
		 (elder-parse-csh-list
		  (buffer-substring-no-properties firstpos
				    (point-max)))))))))
)
...
;;;====================================================
(defun elder-strip-comments-old (workbuffer olcomment)
...
Compare to elder-strip-comments-all though.."
  (set-buffer workbuffer)
  (let* ((comregex (concat "^" (regexp-quote olcomment))))
    (progn
      (goto-char (point-min))
      (flush-lines comregex))))
	 ...
	 (progn
	   ...
	   (if (null curpos)
	     ...
	     (progn
	       (setq curpos (- curpos cl))
	       (goto-char curpos)
	       (if (string= (buffer-substring-no-properties (- curpos 1) curpos)
			    "
")
...
;;;====================================================
	  ...
	  (progn 
;;;	    (elder-message-shell "\nProcessing " (elder-buffer-substring-no-properties
;;;					 (car firstelbegin)
;;;					 (+ (car firstelbegin) 10)))
	    (goto-char (point-min))
	    (setq cutregionend
...
"[eef], not necessarily this one.\n"
   "Look for dangling [eef]s with no ) \n"
	    ...
	    (let ((expressions (buffer-substring-no-properties 
				(car firstelbegin)
...
; 					 (+ (car firstalias) 10)))
	    ...
	    (let ((aliasname
		   (buffer-substring-no-properties cutregionbegin cutregionend)))
	      (kill-region cutregionbegin cutregionend)
	      (goto-char cutregionbegin)
